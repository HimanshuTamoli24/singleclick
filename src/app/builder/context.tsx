...
function pushUndo(state: BuilderState): BuilderState {
  return {
    ...state,
    undoStack: [...state.undoStack.slice(-29), state.slides.map((s) => ({ ...s, elements: [...s.elements] }))],
    redoStack: [],
  };
}
...
function builderReducer(state: BuilderState, action: BuilderAction): BuilderState {
  switch (action.type) {
    ...
    case "DELETE_SLIDE": {
      ...
      const newState = pushUndo(state);
      const slides = newState.slides.filter((_, i) => i !== action.payload.index);
      const activeSlideIndex = Math.min(newState.activeSlideIndex, slides.length - 1);
    ...
    case "DUPLICATE_SLIDE": {
      ...
      return { ...newState, slides, activeSlideIndex: action.payload.index + 1 };
    }
    ...
    case "SELECT_SLIDE": {
      return { ...state, activeSlideIndex: action.payload.index, selectedElementId: null };
    }
    ...
    case "SET_BACKGROUND": {
      const newState = pushUndo(state);
      const slides = newState.slides.map((s, i) =>
        i === newState.activeSlideIndex ? { ...s, background: action.payload.background } : s,
      );
      return { ...newState, slides };
    }
    ...
    case "ADD_ELEMENT": {
      const newState = pushUndo(state);
      const slides = newState.slides.map((s, i) =>
        i === newState.activeSlideIndex ? { ...s, elements: [...s.elements, action.payload.element] } : s,
      );
      return { ...newState, slides, selectedElementId: action.payload.element.id };
    }
        ...
        i === newState.activeSlideIndex
          ? {
              ...
              elements: s.elements.map((el) =>
                el.id === action.payload.id ? ({ ...el, ...action.payload.updates } as SlideElement) : el,
              ),
            }
          : s,
      );
    ...
    case "DELETE_ELEMENT": {
      ...
      const slides = newState.slides.map((s, i) =>
        i === newState.activeSlideIndex
          ? { ...s, elements: s.elements.filter((el) => el.id !== action.payload.id) }
          : s,
      );
    ...
    case "DUPLICATE_ELEMENT": {
      ...
      if (!source) return state;
      const dup: SlideElement = { ...source, id: generateId(), x: source.x + 20, y: source.y + 20, name: `${source.name} copy` } as SlideElement;
      const slides = newState.slides.map((s, i) =>
        i === newState.activeSlideIndex ? { ...s, elements: [...s.elements, dup] } : s,
      );
    ...
    case "PASTE_ELEMENT": {
      ...
      const newState = pushUndo(state);
      const pasted: SlideElement = { ...state.clipboard, id: generateId(), x: state.clipboard.x + 20, y: state.clipboard.y + 20 } as SlideElement;
      const slides = newState.slides.map((s, i) =>
        i === newState.activeSlideIndex ? { ...s, elements: [...s.elements, pasted] } : s,
      );
    ...
    case "UNDO": {
      ...
      return {
        ...
        slides: prevSlides,
        undoStack,
        redoStack: [...state.redoStack, state.slides.map((s) => ({ ...s, elements: [...s.elements] }))],
        selectedElementId: null,
      };
    }
    ...
    case "REDO": {
      ...
      return {
        ...
        slides: nextSlides,
        redoStack,
        undoStack: [...state.undoStack, state.slides.map((s) => ({ ...s, elements: [...s.elements] }))],
        selectedElementId: null,
      };
    }

    case "SET_ZOOM":
      return { ...state, zoom: Math.max(25, Math.min(200, action.payload.zoom)) };
    ...
    case "TOGGLE_PREVIEW":
      return { ...state, previewMode: !state.previewMode, selectedElementId: null };
      ...
      const slides = newState.slides.map((s, i) =>
        i === newState.activeSlideIndex
          ? {
              ...s,
              elements: s.elements.map((el) =>
                el.id === action.payload.id ? ({ ...el, visible: !el.visible } as SlideElement) : el,
              ),
            }
          : s,
      );
      ...
      const slides = newState.slides.map((s, i) =>
        i === newState.activeSlideIndex
          ? {
              ...s,
              elements: s.elements.map((el) =>
                el.id === action.payload.id ? ({ ...el, locked: !el.locked } as SlideElement) : el,
              ),
            }
          : s,
      );
...
export function BuilderProvider({
  ...
  const [state, dispatch] = useReducer(builderReducer, {
    ...initialState,
    slides: initialSlides && initialSlides.length > 0 ? initialSlides : initialState.slides,
  });

  const activeSlide = state.slides[state.activeSlideIndex];
  const selectedElement = activeSlide?.elements.find((el) => el.id === state.selectedElementId);
    ...
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (e.key === "Delete" || e.key === "Backspace") {
        if (state.selectedElementId) {
          e.preventDefault();
          dispatch({ type: "DELETE_ELEMENT", payload: { id: state.selectedElementId } });
        }
      }
      ...
      if (isCtrl && e.key === "d") {
        e.preventDefault();
        if (state.selectedElementId) {
          dispatch({ type: "DUPLICATE_ELEMENT", payload: { id: state.selectedElementId } });
        }
      }
      if (isCtrl && e.key === "c") {
        e.preventDefault();
      ...
      if (state.selectedElementId && ["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(e.key)) {
        ...
        const el = activeSlide?.elements.find((el) => el.id === state.selectedElementId);
        if (el) {
          dispatch({
            type: "UPDATE_ELEMENT",
            payload: {
  ...
  return (
    <BuilderContext.Provider value={{ state, dispatch, activeSlide, selectedElement }}>
      {children}
